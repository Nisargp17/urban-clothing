...
export default function AdminProducts() {
  ...
  const handleOpenAdd = () => {
    setForm({ title: '', newPrice: '', oldPrice: '', stock: '', category: 'mens', tags: '', img: '' });
    setEditingId(null);
    setShowForm(true);
  };
      ...
      {showForm && (
        <div className="border-2 border-[#2a2520] bg-white p-5 shadow-[4px_4px_0px_0px_#2a2520]">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="text-[10px] tracking-[0.15em] text-[#2a2520]/50 block mb-1">IMAGE URL</label>
              <input
                required
                value={form.img}
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.map((product) => (
              <tr key={product.id} className="border-b border-[#2a2520]/5 hover:bg-[#f5efe6] transition-colors">
                <td className="p-4">
